 ...
 *
 * Fluxo:
 *   1. Le o último entrega aberto do case que aponta para _pontes/kanboard.
 ...
 *   5. Atualiza o entrega com resultado_bridge (card_id, link) e status=done.
 */
...
export async function criarTarefa(client: JsonRpcClient, args: CliArgs): Promise<void> {
  ...
  // V0: leitura / escrita do entrega e feita pelo agente Claude que invoca
  // a operação, não por este script. Este script foca em validar payload,
  // pedir HITL e tocar a API. O Claude le o entrega, monta o payload,
  // executa este script com o payload como variavel de ambiente / stdin,
  // e re-escreve o entrega com resultado_bridge. Padrão adaptado de Austin:
  ...
  console.log("\nAtualize o entrega com resultado_bridge:");

...
Bridge TypeScript que fala com o Kanboard institucional. NÃO e especialista. E infraestrutura -- módulos chamados pelo `01-gestor/` após HITL aprovado.
...
3. Se `y`: chama a API.
4. Se `n`: aborta, registra motivo no entrega.
...
```
bun run _pontes/kanboard/facade.ts criar-tarefa --case CASO-2026-0001-validacao-v0
```

O facade le o entrega aberto do case que aponta para `_pontes/kanboard` e executa a operação com HITL.

## Tratamento de falha

- **Falha de rede / API offline:** registra erro no entrega (`status: open` mantido, `error_log` adicionado em payload). Gestor retentea quando rede voltar.

...
- Não gere comunicação em nome da CGTE antes de `_config/voice/cgte.md` estar preenchida. Bloqueia a chain.
...
- **MOOC** -> `02_educacao/` (V1+)
- **Conteúdo Educacional** -> `02_educacao/` (V1+)
- **Formação e Capacitação** -> `02_educacao/` (V1+)
- **Programação Visual Educacional** -> `02_educacao/` (V1+)
- **Produção Audiovisual** -> `03_audiovisual/` (V1+)
- **Evento / Transmissão** -> `03_audiovisual/` (V1+)
- **Comunicação Visual** -> `04_comunicacao/` (V1+)
- **Conteúdo Digital** -> `04_comunicacao/` (V1+)
...
- **Inteligência Artificial** -> `06_tech_sistemas/` (V1+)
- **Gestao / PGD** -> `07_institucional/` (V1+)
- **Comissão** -> `07_institucional/` (V1+)
- **Colaboracao Institucional** -> `07_institucional/` (V1+)
- **Produção Científica** -> `08_ciencia/` (V1+)
...
- Se criar nova categoria: atualize também `_config/business-rules.md` (esta seção) e `01_gestor/reference/taxonomia-board-47.md`.
...
- **Friday review** -- toda sexta. Vai em `ops/friday-review.md`. Inclui: cases ativos, padrões que surgiram, sugestões de chain nova, e o que esta puxando o gestor para fora do papel.
- **Onboarding de especialista (V1.X)** -- ~3h. Servidor + gestor preenchem `identity.md`, `rules.md`, `examples.md`, `handoff.md` do especialista da área. Sem essa sessão, a pasta fica esqueleto.

...
- **Confira o 3-line plan do orchestrator antes de aceitar o entrega.** Se faltar, devolva. O gestor não decide sem ver o raciocínio.
- **Mapeie categoria do board 47 explicitamente.** Use `../referencias/taxonomia-board-47.md`. Não inferir "deve ser tipo isso aqui".
- **Antes de chamar `../../_pontes/kanboard/`, preencha o `kanboard_card_request` completo.** Título, descrição, categoria, responsável, prazo, coluna destino. Cada campo deliberado.
...
- **Registrar `hitl_aprovado_em` e `hitl_aprovado_por` no payload.** Mesmo se for só eu, o registro vai no entrega YAML para auditoria git.
- **Atualizar o case** com o entrega novo, o link para o card criado (após retorno do bridge), e qualquer decisão tomada (ex: nova categoria criada).
- **Para `demanda_extraordinaria`: registrar a decisão.** Criou nova categoria? Atualize também `../../_configuracao/regras-negocio.md` e `../referencias/taxonomia-board-47.md`. Recusou? Por que. Redirecionou? Para onde.

## Nunca

- **Nunca aprove HITL sem ler o payload.** Volta para "Sempre" -- esse é o ponto da V0.
- **Nunca pule o `../../_pontes/kanboard/` para chamar a API direto.** O bridge existe para o HITL acontecer no mesmo lugar sempre. Pular = perder o ponto.
...
- **Nunca escreva em nome da CGTE em comentário / email institucional antes de `../../_configuracao/voz/cgte.md` estar preenchida.** Bloqueio explícito, não "deixar passar essa".
...
Você lê. Se OK, `y`. Se algo errado, `editar` (volta para preencher) ou `n` (cancela e registra o motivo no entrega).

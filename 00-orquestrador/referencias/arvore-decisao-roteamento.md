...
Heurística para escolher a chain certa por tipo de demanda. Use quando a escolha não for óbvia só olhando `cadeias-fluxo.yaml`.

## Passo 1: A demanda é um tema novo ou já tem case?

**Case existente** (já existe `casos/CASO-YYYY-NNNN-shortslug/` para o tema):
- Leia o entrega mais recente do case.
...
**Tema novo** (nenhum `id_caso` existe):
- Continue para o Passo 2.

## Passo 2: A demanda casa com alguma categoria do board 47?

A taxonomia está em `01-gestor/referencias/taxonomia-board-47.md`. Os 8 eixos:
...
### Demanda duplicada
Antes de criar case novo, busque em `casos/` por casos com tema parecido nos últimos 30 dias. Se encontrar um `active`, anexe um entrega novo ao case existente em vez de criar outro. O `id_entrega` continua a sequência (EN-002, EN-003) dentro do case original.
...
- Não é exaustiva. Cobre casos comuns. Edge cases aparecem; documente em `operacoes/revisao-sexta.md` e atualize esta árvore conforme padrões emergirem.
- Não substitui `_configuracao/cadeias-fluxo.yaml`. As chains definem quais transições são válidas. Esta árvore ajuda a escolher qual chain entrar quando o input é ambíguo.

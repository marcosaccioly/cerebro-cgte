...
2. Mapeamento real de IDs do Kanboard (entram em `_pontes/kanboard/projetos-cgte.yaml` e `usuarios-cgte.yaml`).
...
- Default: `47`
- Tipo: numerico
- Nota: V0 assume projeto 47 ("CGTE - Atividades"). Se mudou, atualize também `_configuracao/regras-negocio.md` e `_pontes/kanboard/projetos-cgte.yaml`.
...
- Tipo: sim / não
- Se NÃO: deixe `_configuracao/voz/cgte.md` e `_configuracao/voz/marquito.md` como estão (placeholder com nota BLOQUEADOR). Marque sessão para a próxima semana e atualize este questionario com a data.
...
- Tipo: texto livre
- Nota: se for migrar para `https://gitlab.ifes.edu.br/cefor/cerebro-cgte.git`, escreva o GitLab. O bridge `_pontes/gitlab/sincronizar.ts` funciona com qualquer remote.

### Q8: Primeira chamada do bridge ja rodou? (para preencher IDs reais)

- Tipo: sim / não
- Se NÃO: o agente roda `bun run _pontes/kanboard/facade.ts listar-tarefas-projeto --projeto 47` com as credenciais preenchidas, captura categorias / colunas / usuarios reais, e popula `_pontes/kanboard/projetos-cgte.yaml` e `usuarios-cgte.yaml` substituindo os placeholders.
...
2. Rodar `bun run _pontes/gitlab/sincronizar.ts push`. Como `.env` esta gitignored e voz CGTE provavelmente ainda esta como placeholder, isso vira mudanças não sensiveis -- auto-push.
3. Rodar `bun run _pontes/kanboard/facade.ts listar-tarefas-projeto --projeto 47` para validação end-to-end de credenciais.
4. Abrir `casos/CASO-2026-0001-validacao-v0/caso.md` e walkar pela chain de validação com o agente.
